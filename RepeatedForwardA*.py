...
# Make the grid, having the random origin and destination block set to unblocked and seendef setup():
    grid = makeGrid()
...
def main():
    ...
    while not (s_start.x == s_goal.x and s_start.y == s_goal.y):
        ...
        '''
        while(ptr != None):
            print('track is [{} {}]'.format(ptr.x, ptr.y), destination=' ')
            ptr = ptr.next
        '''
    ...
    '''
    while ptr != None:
        print("path is [{} {}]".format(ptr.x, ptr.y), destination = " ")
        ptr = ptr.next
    '''
